...
export default function GenreSetup() {
      ...
      render: (_, record) => (
        <Space size="middle">
          <EditOutlined 
            ...
            onClick={() => navigate(`/admin/setups/genre/edit/${record.genre_id}`)} 
          />
          <DeleteOutlined 
            className="text-gray-400 hover:text-red-500 cursor-pointer text-lg transition-colors" 
            onClick={() => message.warning(`Delete genre: ${record.genre_name}`)} 
          />
        </Space>
      ),
    },
  ...
  const filteredData = dataSource.filter((item) => item.genre_name.toLowerCase().includes(searchText.toLowerCase()));
    ...
    <div className="p-4">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <Button 
          ...
          onClick={() => navigate('/admin/setups/genre/add')} 
        >
          Add Genre
        </Button>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        <Table 
          ...
          loading={loading} 
          rowKey="genre_id" 
          pagination={{ pageSize: 6, showTotal: (total, range) => <span className="text-gray-400 font-normal">Showing {range[0]} to {range[1]} of {total} results</span> }} 
          className="
