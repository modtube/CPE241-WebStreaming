...
export default function LanguageSetup() {
  ...
  const fetchLanguages = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:5000/api/languages?search=${searchText}`,
      );
  ...
  const columns: ColumnsType<Language> = [
    ...
    },
    {
      ...
      key: "language_name",
      width: "35%",
      sorter: (a, b) => a.language_name.localeCompare(b.language_name),
      render: (text) => (
  ...
  const filteredData = dataSource.filter(
    (item) =>
      item.language_id.toLowerCase().includes(searchText.toLowerCase()) ||
      item.language_name.toLowerCase().includes(searchText.toLowerCase()) ||
      item.native_name.toLowerCase().includes(searchText.toLowerCase()),
  );
  ...
  return (
    <div className="p-4">
      ...
      <GenericAddModal
        ...
        fields={[
          { label: "English Name", name: "language_name", required: true },
          { label: "Native Name", name: "native_name", required: true },
        ]}
      />
    </div>
  );
