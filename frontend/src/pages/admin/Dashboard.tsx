...
const StatCard = ({ title, value, icon, trend, trendColor }: StatCardProps) => (
  <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm flex flex-col gap-2">
    ...
    {trend && (
      <div
        className={`text-xs font-bold ${trendColor || "text-green-500"} flex items-center gap-1`}
      >
        {trend}
      </div>
    )}
    ...
    fetch("http://localhost:5000/api/dashboard")
      ...
      .then((json) => {
        ...
        const processed = {
          ...dashData,
          revenueTrend:
            dashData.revenueTrend?.map((item: any) => ({
              ...item,
              amount: parseFloat(item.amount) || 0,
            })) || [],
          genreDistribution:
            dashData.genreDistribution
              ?.map((item: any) => ({
                ...item,
                value: parseInt(item.value, 10) || 0,
              }))
              .sort((a: any, b: any) => b.value - a.value) || [],
          // 🟢 ข้อมูล User Growth ตอนนี้จะเป็นยอดสะสมที่คำนวณมาจาก SQL แล้ว
          userGrowth:
            dashData.userGrowth?.map((item: any) => ({
              ...item,
              count: parseInt(item.count, 10) || 0,
            })) || [],
          countryDistribution:
            dashData.countryDistribution
              ?.map((item: any) => ({
                ...item,
                value: parseInt(item.value, 10) || 0,
              }))
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-white p-8 rounded-2xl border border-gray-100 shadow-sm">
          ...
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                data={data?.countryDistribution?.slice(0, 10)}
                layout="vertical"
                margin={{ left: 10 }}
              >
                <XAxis type="number" hide />
